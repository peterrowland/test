...
def f06(string1, string2):
    ...
    if len(string1) > len(string2):
        ...
        print('{} is longer than {} by {} characters'.format(string1, string2, diff))
        print('"{}" has only {%2d} the number of chars of "{}".'.format(string2, (len(string2 / string1)), string1))
    else:
        diff = len(string2) - len(string1)
        print('"{}" is longer than "{}" by {} characters'.format(string2, string1, diff))
        print('"{}" has only {:.2%} the number of chars of "{}".'.format(string1, ((len(string1)) / len(string2)), string2))
...
###############################################################################
def f09():
    multiples = [x for x in range(1,500) if x % (3 * 5) == 0]
...
def f10(i):
    ...
    elif i in range(1,501):
        sum_multiples += f10(i + 1)
...
def f12():
    ...
    # log_file
    with open('log_file.txt', 'r+') as err_log:
        for i in errors:
            err_log.write(i, '\n')
